...
if __name__ == '__main__':
    ...
    for cluster in range(0,10):
        ...
        words = []
        centroids_value = list ( word_centroid_map.values() )
        centroids_key = list (word_centroid_map.keys())
        for i in range(0,len(word_centroid_map.values())):
            if( centroids_value[i] == cluster ):
                words.append(centroids_key[i])
        print ( words )
    ...
    train_centroids = np.zeros( (train_data["content"].size, num_clusters), \
        dtype="float32" )
    ...
    test_centroids = np.zeros(( test_data["content"].size, num_clusters), \
        dtype="float32" )
    ...
    error = 0
    num_reviews = len(test_data['content'])
